...
" autocmd BufNewFile,BufEnter *.pde nnoremap <buffer> zb :Shell ~/.vim/scripts/build-processing.sh %:p:S<CR>
autocmd BufNewFile,BufEnter *.pde nnoremap <buffer> zb :execute 'Term ' . expand($HOME) . '/.vim/scripts/build-processing.sh %:p:gs/./\\&/'<CR>
